...
# """
# Flask API Application
# REST API for criminal face detection, recognition, admin security and video processing.

# Drop-in file to replace existing src/api/app.py — copy / paste this file.
# """
...
# from src.mfa_system import MFASystem
...
# from src.matcher import FaceMatcher
# from src.db import Database
# from src.security import ip_whitelist, get_security_status  # security module
...
# app.config["MAX_CONTENT_LENGTH"] = 1024 * 1024 * 1024  # allow larger for video; adjust as needed
...
#         db.log_event("ADD_SUBJECT", subject_id, confidence, {"name": name, "crime": crime})
#         return jsonify({"success": True, "data": {"subject_id": subject_id, "name": name, "crime": crime, "confidence": confidence, "timestamp": datetime.now().isoformat()}})
#     except Exception as e:
...
#                     "top_matches": top_matches,
#                 }
#                 db.log_event("MATCH", match["subject_id"], match["similarity_score"], {"name": match["name"], "crime": match["crime"]}, filepath)
#             else:
#                 match_data = {"face_id": i + 1, "bounding_box": box, "detection_confidence": det_conf, "status": "NO_MATCH", "message": "No matching subject found"}
#                 db.log_event("NO_MATCH", None, None, {}, filepath)
#             matches.append(match_data)

#         response = {"success": True, "data": {"faces_detected": len(results), "matches": matches, "timestamp": datetime.now().isoformat()}}
#         return app.response_class(json.dumps(response, cls=NumpyEncoder), mimetype="application/json")
#     except Exception as e:
...
# # ---------------------------
# # Security admin endpoints (used by admin.html)
# # ---------------------------
...
#     added = ip_whitelist.add_to_whitelist(ip)
#     return jsonify({"success": True, "message": f"{ip} added" if added else f"{ip} already exists"})
...
#     if not ip:
#         return jsonify({"success": False, "error": "Missing IP"}), 400
#     removed = ip_whitelist.remove_from_whitelist(ip)
#     return jsonify({"success": True, "message": f"{ip} removed" if removed else f"{ip} not found"})
...
#     if not ip:
#         return jsonify({"success": False, "error": "Missing IP"}), 400
#     added = ip_whitelist.add_to_blacklist(ip)
#     return jsonify({"success": True, "message": f"{ip} blacklisted" if added else f"{ip} already blacklisted"})
...
#     if not ip:
#         return jsonify({"success": False, "error": "Missing IP"}), 400
#     removed = ip_whitelist.remove_from_blacklist(ip)
#     return jsonify({"success": True, "message": f"{ip} removed from blacklist" if removed else f"{ip} not found"})


# # ---------------------------
# # ---------------------------------------
# # Video Processing Endpoint (Fixed)
# # Add video processing endpoint
...
#         # **FIX: Convert numpy arrays to lists for JSON serialization**
#         # Clean up results to be JSON serializable
#         json_safe_results = {
...
#                 'bounding_box': [float(x) for x in match['bounding_box']]  # Convert to list
#             })
        
#         # Convert timeline (optional, can be large)
#         for event in results['detection_timeline'][:100]:  # Limit to 100 events
#             json_safe_results['detection_timeline'].append({
#                 'frame': int(event['frame']),
#                 'timestamp': float(event['timestamp']),
#                 'faces': int(event['faces']),
...
# # Add live monitoring API endpoint
# @app.route('/api/start_live_monitoring', methods=['POST'])
# def start_live_monitoring():
#     """Start live stream monitoring via API"""
#     try:
...
# @app.route('/api/stop_live_monitoring', methods=['POST'])
...
# # Update app.run() at the bottom to use socketio
# if __name__ == '__main__':
#     socketio.run(
#         app,
#         host='0.0.0.0',
...
#         # Create new monitor with WebSocket callback
#         live_monitor = LiveStreamMonitor()
...
#             logger.info(f"✅ Started monitoring: {stream_url}")
#         else:
