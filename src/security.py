...
class IPWhitelist:
    ...
    def is_ip_allowed(self, ip: str) -> bool:
            ...
            try:
                if "/" in allowed:
                    ...
                    if ipaddress.ip_address(ip) in network:
                        logger.info(f"Allowed IP {ip} (matches {allowed})")
                        return True
                elif allowed == "localhost" and ip in ("127.0.0.1", "::1"):
                    logger.info(f"Allowed localhost: {ip}")
                    return True
                elif ip == allowed:
                    logger.info(f"Allowed IP: {ip}")
                    return True
        ...
        logger.warning(f"Denied IP (not whitelisted): {ip}")
        self.record_failed_attempt(ip)
        return False
    ...
    def record_failed_attempt(self, ip: str):
        ...
        if len(self.failed_attempts[ip]) >= self.max_failed_attempts:
            self.add_to_blacklist(ip)
            logger.warning(f"{ip} auto-blacklisted after {self.max_failed_attempts} failed attempts")
    ...
    def remove_from_blacklist(self, ip: str) -> bool:
        if ip in self.blacklist:
            self.blacklist.remove(ip)
            self.save_whitelist()
            logger.info(f"Removed from blacklist: {ip}")
            return True
        return False
...
class AuthenticationSystem:
    """
    Minimal authentication store (file-backed)
    """
    ...
    def load_users(self):
        try:
            if os.path.exists(self.auth_file):
                ...
                default = self.hash_password("admin123")
                self.users = {
                    "admin": {"password": default, "role": "admin", "created": datetime.now().isoformat()}
                }
                self.save_users()
                logger.warning("Created default admin user (admin/admin123) - change immediately")
        except Exception as e:
            logger.error(f"Error loading users: {e}")
    ...
    def create_session(self, username: str) -> str:
        token = secrets.token_urlsafe(32)
        self.sessions[token] = {"username": username, "created": datetime.now(), "expires": datetime.now() + timedelta(seconds=self.session_duration)}
        return token
    ...
    def add_user(self, username: str, password: str, role: str = "user"):
        self.users[username] = {"password": self.hash_password(password), "role": role, "created": datetime.now().isoformat()}
        self.save_users()
    ...
    def decorated(*args, **kwargs):
        ...
        if not ip_whitelist.is_ip_allowed(client_ip):
            ...
            return render_template_string(
                ...
                <h1>Access Denied</h1><p>Your IP: {{ ip }}</p>
                </body></html>
                """,
                ip=client_ip,
            ), 403
...
def log_access(f):
    ...
    def decorated(*args, **kwargs):
        client_ip = ip_whitelist.get_client_ip(request)
        os.makedirs("logs", exist_ok=True)
        entry = {"timestamp": datetime.now().isoformat(), "ip": client_ip, "method": request.method, "path": request.path, "ua": request.user_agent.string}
...
if __name__ == "__main__":
    ...
    print("IPWhitelist quick test")
    print("127.0.0.1 allowed:", ip_whitelist.is_ip_allowed("127.0.0.1"))
